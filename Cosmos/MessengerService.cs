...
public class MessengerService : IMessengerService
{
	...
	private readonly IRepository<Channel> channelRepo;
	private readonly EventService         eventService;
	private readonly IRepository<Message> messageRepo;

	public MessengerService(IRepository<Account> accountRepo,
	...
	public async Task InitializeAsync()
	{
		try
		{
			await channelRepo.CreateNew(new Channel([CactusConstants.EveryoneId], CactusConstants.GlobalChannelId,
			                                        "Global Channel"));
		}
		catch { }
	}
	...
	public async Task<Guid> CreateChannel(HashSet<Guid> userIds, Guid userId, string name)
	{
		...
		throw new UnauthorizedAccessException(
			"User must be member of the channel (Except the user has administrative permissions)");
	}
	...
	public async Task AddUserToChannel(Guid Id, Guid channelId, Guid userId)
	{
		...
		{
			throw new UnauthorizedAccessException(
				"Can only add users to channels you are in (Except for Admin accounts)");
		}
	}
	...
	public async Task DeleteAccount(Guid id)
	{
		...
		await Task.WhenAll(channelIds
			                   .Select(channelId => { return removeUserFromChannel(channelId, id, id); }));
		eventService.ChannelsHaveChanged();
	...
	public async Task EditAccountAdmin(Guid id, bool giveAdmin, Guid userId)
	{
		...
		{
			throw new UnauthorizedAccessException(
				"No permission (Only admins can change the admin status of other users)");
		}
	}
	...
	public async Task EditAccountEmail(Guid id, string email, Guid userId)
	{
		...
		{
			throw new UnauthorizedAccessException(
				$"Acces denied (Only admins or the owner of the account with ID {id} can change the email adress of this account.");
		}
	}
	...
	public async Task ChangePW(Guid Id, Guid userId, string newPW)
	{
		...
		{
			throw new UnauthorizedAccessException(
				"Acces denied (Only the owner of the account/admins can edit the password of an account)");
		}
	}
